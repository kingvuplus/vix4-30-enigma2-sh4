...
class EPGSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session, service = None, zapFunc = None, eventid = None, bouquetChangeCB = None, serviceChangeCB = None, EPGtype = None, StartBouquet = None, StartRef = None, bouquets = None):
        ...
        if self.type == EPG_TYPE_GRAPH:
            self['colouractions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.redButtonPressed, _('Open option menu')),
             ...
             'blue': (self.blueButtonPressed, _('Go forward by 24 hours'))}, -1)
        else:
            self['colouractions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.redButtonPressed, _('IMDB  for current event')),
        ...
        if self.type == EPG_TYPE_SIMILAR:
            ...
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.prevPage, _('Move up a page')),
             ...
             'down': (self.moveDown, _('Goto next channel'))}, -1)
            ...
            if self.type == EPG_TYPE_INFOBAR:
                ...
                self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextBouquet': (self.nextBouquet, _('Goto next bouquet')),
                 ...
                 'menu': (self.createSetup, _('Setup menu'))}, -1)
                self['epgactions'].csel = self
                self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.prevService, _('Goto previous channel')),
            ...
            self['bouquetcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.moveBouquetPageUp, _('Goto previous event')),
             'right': (self.moveBouquetPageDown, _('Goto next event')),
             'up': (self.moveBouquetUp, _('Goto previous channel')),
             'down': (self.moveBouquetDown, _('Goto next channel'))}, -1)
            self['bouquetcursoractions'].csel = self
            self['bouquetcursoractions'].setEnabled(False)
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (self.leftPressed, _('Goto previous event')),
             'right': (self.rightPressed, _('Goto next event')),
             'up': (self.moveUp, _('Goto previous channel')),
             'down': (self.moveDown, _('Goto next channel'))}, -1)
            self['epgcursoractions'].csel = self
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextService': (self.prevPage, _('Page Up')),
    ...
    def onSelectionChanged(self):
        ...
        elif isRecordEvent and self.key_green_choice != self.REMOVE_TIMER:
            self['key_green'].setText(_('Change timer'))
            self.key_green_choice = self.REMOVE_TIMER
        elif not isRecordEvent:
            self['key_green'].setText(_('Add Timer'))
        ...
        if self.changeText != self.EMPTY:
            ...
            menu = [(_('IMDb Search'),
             ...
             (_('Search'),
              ...
              self.openEPGSearch)]
            title = '%s?' % event.getEventName()
            self.ChoiceBoxDialog = self.session.instantiateDialog(ChoiceBox, title=title, list=menu, keys=['1',
             '2',
             '3',
             '4',
             '5'], skin_name='OptionDialog')
            serviceref = eServiceReference(str(self['list'].getCurrent()[1]))
            posy = self['list'].getSelectionPosition(serviceref)
